...
const getCareerTechnicalSkills = async (code) => {
    const careerTechnicalSkillsUrl =
    ...
    try {
        ...
        if (data.hasOwnProperty("category")) {
            ...
            technical_skills = technical_skills.map((skill) =>
                skill.example.map((el) => el.name)
            );
    ...
    try {
        ...
        if (result.hasOwnProperty("keyword")) {
            ...
            result = result.hasOwnProperty("occupation")
                ? result.occupation
                      .filter((res) => res.relevance_score > relevance_score)
                      .map((res) => {
                          return { code: res.code, title: res.title };
                      })
                : null;
        } else {
            return null;
